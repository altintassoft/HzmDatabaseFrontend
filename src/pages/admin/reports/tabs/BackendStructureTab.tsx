...
interface BackendStructureTabProps {
...
function parseBackendFiles(markdown: string): FileInfo[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Format: | 1 | `admin.js` | 2416 | `Backend/src/routes/admin.js` | 🚨 **ACİL MÜDAHALE GEREKLİ** |
...
function buildDirectoryTree(files: FileInfo[]): DirectoryNode {
  ...
  files.forEach(file => {
    // Path'i parçala: Backend/src/routes/admin.js -> ['Backend', 'src', 'routes', 'admin.js']
    const parts = file.path.split('/');
    let currentNode = root;
    
    // İlk 'Backend' kısmını atla
...
export default function BackendStructureTab({ markdownContent }: BackendStructureTabProps) {
  const files = parseBackendFiles(markdownContent);
